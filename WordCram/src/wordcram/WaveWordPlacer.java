...
public class WaveWordPlacer extends WordPlacer {
	...
	public PlaceInfo place(Word word, int wordIndex, int wordsCount,
		...
		return new PlaceInfo(new PVector(x, y + yOffset), null);
	}

	private float getYOffset(int wordIndex, int wordsCount, int fieldHeight) {
		float theta = PApplet.map(wordIndex, 0, wordsCount, PConstants.PI,
				-PConstants.PI);
