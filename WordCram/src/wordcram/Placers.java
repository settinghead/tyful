	...
	public static WordPlacer horizLine() {
		...
		return new WordPlacer() {
			public PlaceInfo place(Word word, int wordIndex, int wordsCount,
				...
				float xOff = (float) r.nextGaussian()
						* ((fieldWidth - wordImageWidth) * 0.2f);
				float yOff = (float) r.nextGaussian() * 50;

				return new PlaceInfo(centerVertLine + xOff, centerHorizLine
						+ yOff);
				/*
				 * int adjFieldWidth = fieldWidth - wordImageWidth; int
				 * adjFieldHeight = fieldHeight - wordImageHeight;
				 * 
				 * float xOff = (float) r.nextGaussian();// * 0.2f; float yOff =
				 * (float) r.nextGaussian();// * 0.5f; yOff =
				 * (float)Math.pow(yOff, 3) * 1.5f;
				 * 
				 * return new PVector(PApplet.map(xOff, -2, 2, 0,
				 * adjFieldWidth), PApplet.map(yOff, -2, 2, 0, adjFieldHeight));
				 */
			}
	...
	public static WordPlacer centerClump() {
		...
		return new WordPlacer() {
			...
			public PlaceInfo place(Word word, int wordIndex, int wordsCount,
					int wordImageWidth, int wordImageHeight, int fieldWidth,
					int fieldHeight) {
				return new PlaceInfo(getOneUnder(fieldWidth - wordImageWidth),
	...
	public static WordPlacer horizBandAnchoredLeft() {
		...
		return new WordPlacer() {
			public PlaceInfo place(Word word, int wordIndex, int wordsCount,
					int wordImageWidth, int wordImageHeight, int fieldWidth,
					int fieldHeight) {
				float x = (1 - word.weight) * fieldWidth * r.nextFloat(); // big=left,
																			// small=right
				float y = ((float) fieldHeight) * 0.5f;
				return new PlaceInfo(x, y);
			}
