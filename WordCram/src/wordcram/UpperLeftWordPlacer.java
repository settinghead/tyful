...
public class UpperLeftWordPlacer extends WordPlacer {
	...
	public PlaceInfo place(Word word, int wordIndex, int wordsCount,
		...
		return new PlaceInfo(new PVector(x, y), null);
	}

	private int getOneUnder(int limit) {
		return PApplet.round(PApplet.map(
				random(random(random(random(random(1.0f))))), 0, 1, 0, limit));
	}
