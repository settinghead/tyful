...
//      new Exception().printStackTrace();
  ...
  protected void parseLine() {
    ...
    family = PRIMITIVE;
    params = new float[] {
      getFloatWithUnit(element, "x1"), getFloatWithUnit(element, "y1"),
  ...
  protected void parseRect() {
    ...
    family = PRIMITIVE;
    params = new float[] {
      getFloatWithUnit(element, "x"), getFloatWithUnit(element, "y"),
...
//    char prevCommand = '\0';
    ...
    while (i < pathTokens.length) {
      ...
      default:
        String parsed = PApplet.join((String[]) PApplet
            .subset(pathTokens, 0, i), ",");
        String unparsed = PApplet
            .join((String[]) PApplet.subset(pathTokens, i), ",");
        ...
        throw new RuntimeException("shape command not handled: "
            + pathTokens[i]);
      }
//      prevCommand = c;
    }
...
//      }
  ...
  private void parsePathCode(int what) {
    if (vertexCodeCount == vertexCodes.length) {
      vertexCodes = (int[]) PApplet.expand(vertexCodes);
    }
  ...
  private void parsePathCurveto(float x1, float y1, float x2, float y2,
                                float x3, float y3) {
...
//    parsePathCode(BEZIER_VERTEX);
   ...
   * More about the transformation matrices in <a
  ...
  void setColor(String colorText, boolean isFill) {
    ...
    if (colorText.equals("none")) {
      ...
      color = opacityMask | (Integer.parseInt(colorText.substring(1), 16))
          & 0xFFFFFF;
...
//        visible = false;
  ...
  static class Gradient extends PShapeSVG {
    ...
    public Gradient(PShapeSVG parent, XML properties) {
      ...
      offset = (float[]) PApplet.subset(offset, 0, count);
        ...
        if (span <= 0) {
          ...
          for (int i = 1; i < count; i++) {
            ...
            for (int j = prev; j <= last; j++) {
              ...
              interp[j][0] = (int) PApplet.lerp((c0 >> 16) & 0xff,
                                                (c1 >> 16) & 0xff, btwn);
              interp[j][1] = (int) PApplet.lerp((c0 >> 8) & 0xff,
                                                (c1 >> 8) & 0xff, btwn);
              interp[j][2] = (int) PApplet.lerp(c0 & 0xff, c1 & 0xff, btwn);
              interp[j][3] = (int) (PApplet.lerp((c0 >> 24) & 0xff,
                                                 (c1 >> 24) & 0xff, btwn) * opacity);
  ...
  protected Paint calcGradientPaint(Gradient gradient) {
    if (gradient instanceof LinearGradient) {
      ...
      return new RadialGradientPaint(grad.cx, grad.cy, grad.r, grad.offset,
                                     grad.color, grad.count, opacity);
    }
    return null;
  }
    ...
    public FontGlyph(PShapeSVG parent, XML properties, Font font) {
      ...
      if (u != null) {
        if (u.length() == 1) {
          ...
          System.err.println("unicode for " + name + " is more than one char: "
              + u);
        }
      }
