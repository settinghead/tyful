 ...
 * 
 * <p>
 * Pixel operations too slow? As of release 0085 (the first beta), the default
 * renderer uses Java2D. It's more accurate than the renderer used in alpha
 * releases of Processing (it handles stroke caps and joins, and has better
 * polygon tessellation), but it's super slow for handling pixels. At least
 * until we get a chance to get the old 2D renderer (now called P2D) working in
 * a similar fashion, you can use <TT>size(w, h, P3D)</TT> instead of
 * <TT>size(w, h)</TT> which will be faster for general pixel flipping madness.
 * </p>
 * 
 * <p>
 ...
 * </PRE>
 * 
 * This will let you do Java 2D stuff directly, but is not supported in any way
 * shape or form. Which just means "have fun, but don't complain if it breaks."
 * </p>
 */
...
//      image = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
  ...
  public void vertex(float x, float y) {
    ...
    case TRIANGLES:
      if ((vertexCount % 3) == 0) {
        triangle(vertices[vertexCount - 3][X], vertices[vertexCount - 3][Y],
                 vertices[vertexCount - 2][X], vertices[vertexCount - 2][Y], x,
                 y);
      }
      break;
    ...
    case TRIANGLE_STRIP:
      if (vertexCount >= 3) {
        triangle(vertices[vertexCount - 2][X], vertices[vertexCount - 2][Y],
                 vertices[vertexCount - 1][X], vertices[vertexCount - 1][Y],
    ...
    case TRIANGLE_FAN:
      if (vertexCount == 3) {
        triangle(vertices[0][X], vertices[0][Y], vertices[1][X],
                 vertices[1][Y], x, y);
        ...
        gpath
            .lineTo(vertices[vertexCount - 2][X], vertices[vertexCount - 2][Y]);
        gpath.lineTo(x, y);
        drawShape(gpath);
      }
      break;
    ...
    case QUADS:
      if ((vertexCount % 4) == 0) {
        quad(vertices[vertexCount - 4][X], vertices[vertexCount - 4][Y],
             vertices[vertexCount - 3][X], vertices[vertexCount - 3][Y],
             vertices[vertexCount - 2][X], vertices[vertexCount - 2][Y], x, y);
      }
      break;
  ...
  public void bezierVertex(float x1, float y1, float x2, float y2, float x3,
                           float y3) {
    bezierVertexCheck();
  ...
  public void quadraticVertex(float ctrlX, float ctrlY, float endX, float endY) {
    ...
    bezierVertex(x1 + ((ctrlX - x1) * 2 / 3.0f),
                 y1 + ((ctrlY - y1) * 2 / 3.0f), endX
                     + ((ctrlX - endX) * 2 / 3.0f), endY
                     + ((ctrlY - endY) * 2 / 3.0f), endX, endY);
  }

  public void quadraticVertex(float x2, float y2, float z2, float x4, float y4,
                              float z4) {
  ...
  protected void curveVertexSegment(float x1, float y1, float x2, float y2,
    ...
    gpath.curveTo(curveDrawX[1], curveDrawY[1], curveDrawX[2], curveDrawY[2],
...
//                        RenderingHints.VALUE_INTERPOLATION_BILINEAR);
   ...
   */
  protected void imageImpl(PImage who, float x1, float y1, float x2, float y2,
                           int u1, int v1, int u2, int v2) {
    ...
    if ((tint && !cash.tinted) || (tint && (cash.tintedColor != tintColor))
        || (!tint && cash.tinted)) {
    ...
    g2.drawImage(((ImageCache) who.getCache(this)).image, (int) x1, (int) y1,
                 (int) x2, (int) y2, u1, v1, u2, v2, null);
  }
          ...
          for (int y = 0; y < source.height; y++) {
            if (source.format == RGB) {
              ...
              for (int x = 0; x < source.width; x++) {
                ...
                int g1 = (argb1 >> 8) & 0xff;
                int b1 = (argb1) & 0xff;
                tintedPixels[x] = alpha | (((r2 * r1) & 0xff00) << 8)
              ...
              for (int x = 0; x < source.width; x++) {
                ...
                int g1 = (argb1 >> 8) & 0xff;
                int b1 = (argb1) & 0xff;
                tintedPixels[x] = (((a2 * a1) & 0xff00) << 16)
                    | (((r2 * r1) & 0xff00) << 8) | ((g2 * g1) & 0xff00)
                    | (((b2 * b1) & 0xff00) >> 8);
              }
...
//    if (font != null && (textFont.isStream() || hints[ENABLE_NATIVE_FONTS])) {
    if (font != null) {
       ...
       * different from the smooth() function, because the font smoothing // is
       * controlled when the font is created, not now as it's drawn. // fixed a
       * bug in 0116 that handled this incorrectly. Object textAntialias =
  ...
  public PMatrix2D getMatrix(PMatrix2D target) {
    ...
    target.set((float) transform[0], (float) transform[2],
               (float) transform[4], (float) transform[1],
               (float) transform[3], (float) transform[5]);
    return target;
  }
  ...
  public void setMatrix(PMatrix2D source) {
    g2.setTransform(new AffineTransform(source.m00, source.m10, source.m01,
                                        source.m11, source.m02, source.m12));
  }
  ...
  public void loadPixels() {
    ...
    WritableRaster raster = ((BufferedImage) (primarySurface ? offscreen
        : image)).getRaster();
...
//    ((BufferedImage) image).setRGB(x, y, argb);
  ...
  public void copy(int sx, int sy, int sw, int sh, int dx, int dy, int dw,
    ...
    if ((sw != dw) || (sh != dh)) {
      ...
      g2.drawImage(img, dx, dy, dx + dw, dy + dh, sx, sy, sx + sw, sy + sh,
                   null);
  ...
  public void copy(PImage src, int sx, int sy, int sw, int sh, int dx, int dy,
                   int dw, int dh) {
    g2.drawImage(src.getImage(), dx, dy, dx + dw, dy + dh, sx, sy, sx + sw, sy
        + sh, null);
  }
