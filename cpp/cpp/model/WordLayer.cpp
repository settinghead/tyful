...
, type(WORD_LAYER), colorSheet(NULL),_angler(NULL),_colorer(NULL),tolerance(1.0){
    
}

WordLayer::WordLayer(unsigned int const * pixels, int width, int height, int lid, bool revert)
:PolarLayer::PolarLayer(pixels,width,height,lid,revert), type(WORD_LAYER), colorSheet(NULL),_angler(NULL),_colorer(NULL),tolerance(1.0){
}
