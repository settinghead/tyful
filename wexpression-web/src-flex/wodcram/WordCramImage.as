 ...
 * 
 * Give WordCram some text to chew on, or an array of Words you've weighted
 * yourself.
 * 
 * <h3>Let WordCram Weight Your Words</h3>
 * 
 * <p>
 * WordCram weights your words by the number of times they appear in a document.
 * It can load the document a few ways:
 * 
 * <ul>
 * <li>{@link #fromWebPage(String)} and {@link #fromHtmlFile(String)} load the
 * HTML and scrape out the words</li>
 * <li>{@link #fromHtmlString(String...)} takes a String (or String[]), assumes
 * it's HTML, and scrapes out its text</li>
 * <li>{@link #fromTextFile(String)} loads a file (from the filesystem or the
 * network), and counts the words</li>
 * <li>{@link #fromTextString(String...)} takes a String (or String[]), and
 * counts the words</li>
 * </ul>
 * 
 * <p>
 * If you need some other way to load your text, pass your own TextSource to
 * {@link #fromText(TextSource)}, and WordCram get its text via
 * {@link TextSource#getText()}.
 * 
 * <p>
 ...
 * <li>{@link #keepCase()}, the default, will count them separately, as three
 * different words</li>
 * </ul>
 * 
 * <p>
 * <b>Numbers:</b> If your text contains words like "42" or "3.14159", you can
 * remove them with {@link #excludeNumbers()} (the default), or include them
 * with {@link #includeNumbers()}.
 * 
 * <p>
 * <b>Stop words:</b> <a
 * href="../constant-values.html#wordcram.text.StopWords.ENGLISH">Common English
 * words</a> are removed from the text by default, but you can use your own list
 * of stop words with {@link #withStopWords(String)}.
 * 
 * 
 * <h3>Weight Your Own Words</h3>
 * 
 * <p>
 * If you have some other way to weight your words, you can pass them to
 * {@link #fromWords(Word[])}, and in that case, you can use
 * {@link Word#setColor(int)}, {@link Word#setFont(PFont)},
 * {@link Word#setAngle(float)}, and/or {@link Word#setPlace(PVector)} to
 ...
 * 
 * There are six questions you have to answer when drawing a word on the
 * WordCram:
 * 
 * <h3>How big should it be?</h3>
 * A word can be {@link #sizedByWeight(int, int)},
 * {@link #sizedByRank(int, int)}, or {@link #withSizer(WordSizer)}
 * 
 * <h3>How should it be angled?</h3>
 * It can be {@link #angledAt(float...)}, {@link #angledBetween(float, float)},
 * or {@link #withAngler(WordAngler)}
 * 
 * <h3>What font should it be in?</h3> You can render words
 * {@link #withFont(String)} or {@link #withFonts(String...)} (those both can
 * also take PFonts), or {@link #withFonter(WordFonter)}
 * 
 * <h3>How should it be colored?</h3>
 * {@link #withColor(int)}, {@link #withColors(int...)}, or
 ...
 * {@link #hasMore()} words to draw (probably once per Processing frame):
 * 
 * <pre>
 * void draw() {
 * 	if (wordCram.hasMore()) {
 ...
 * 
 * <p>
 * If you're having trouble getting your words to show up, you might want to
 * {@link #getSkippedWords()}. Knowing which words were skipped, and why (see
 * {@link Word#wasSkippedBecause()}), can help you size and place your words
 * better.
 * 
 * <p>
 * You can also {@link #getWords()} to see the whole list, and
 * {@link #getWordAt(float,float)} to see which word covers a given pixel.
 * 
 * @author Dan Bernier
 */
public class WordCramImage {
	...
	private var excludeNumbers:Boolean= true;

	private enum TextCase {
		Lower, Upper, Keep
	};

	private var textCase:TextCase= TextCase.Keep;

	private var wordCramEngine:WordCramEngine;

	private var parent:PApplet= new PApplet();
	...
	private var destination:PGraphics= null;
	 ...
	 * 
	 * @param parent
	 *            Your Processing sketch. Pass it as <code>this</code>.
	 */
	public function WordCramImage() {
		parent.init();
		destination = parent.createGraphics(800, 600, PApplet.JAVA2D);
	}
	 ...
	 */
	public function withStopWords(extraStopWords:String):WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function excludeNumbers():WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function includeNumbers():WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function lowerCase():WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function upperCase():WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function keepCase():WordCramImage {
		this.textCase = TextCase.Keep;
		return this;
	}

	/**
	 * Make a WordCram from the text on a web page. Just before the WordCram is
	 * drawn, it'll load the web page's HTML, scrape out the text, and count and
	 * sort the words.
	 * 
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function fromWebPage(webPageAddress:String):WordCramImage {
		return fromText(new WebPage(webPageAddress, parent));
	}

	/**
	 * Make a WordCram from the text in a HTML file. Just before the WordCram is
	 * drawn, it'll load the file's HTML, scrape out the text, and count and
	 * sort the words.
	 * 
	 * @param htmlFilePath
	 *            the path of the html file to load
	 * @return The WordCram, for further setup or drawing.
	 */
	public function fromHtmlFile(htmlFilePath:String):WordCramImage {
	...
	/**
	 * Makes a WordCram from a String of HTML. Just before the WordCram is
	 * drawn, it'll scrape out the text from the HTML, and count and sort the
	 * words. It takes one String, or any number of Strings, or an array of
	 * Strings, so you can easily use it with <a
	 * href="http://processing.org/reference/loadStrings_.html"
	 * target="blank">loadStrings()</a>.
	 * 
	 * @param html
	 *            the String(s) of HTML
	 * @return The WordCram, for further setup or drawing.
	 */
	// example fromHtmlString(loadStrings("my.html"))
	// example fromHtmlString("<html><p>Hello there!</p></html>")
	public function fromHtmlString( ... html):WordCramImage {
		return fromText(new Html(PApplet.join(html, "")));
	}

	/**
	 * Makes a WordCram from a text file, either on the filesystem or the
	 * network. Just before the WordCram is drawn, it'll load the file, and
	 * count and sort its words.
	 * 
	 * @param textFilePathOrUrl
	 *            the path of the text file
	 * @return The WordCram, for further setup or drawing.
	 */
	public function fromTextFile(textFilePathOrUrl:String):WordCramImage {
		return fromText(new TextFile(textFilePathOrUrl, parent));
	}

	/**
	 * Makes a WordCram from a String of text. It takes one String, or any
	 * number of Strings, or an array of Strings, so you can easily use it with
	 * <a href="http://processing.org/reference/loadStrings_.html"
	 * target="blank">loadStrings()</a>.
	 * 
	 * @param text
	 *            the String of text to get the words from
	 * @return The WordCram, for further setup or drawing.
	 */
	// example fromTextString(loadStrings("my.txt"))
	// example fromTextString("Hello there!")
	public function fromTextString( ... text):WordCramImage {
	 ...
	 * 
	 * <p>
	 * It only caches the TextSource - it won't load the text from it until
	 * {@link #drawAll()} or {@link #drawNext()} is called.
	 * 
	 * @param textSource
	 *            the TextSource to get the text from.
	 * @return The WordCram, for further setup or drawing.
	 */
	public function fromText(textSource:TextSource):WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function fromWords(words:Array):WordCramImage {
		this.words = words;
		return this;
	}
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withFonts( ... fontNames):WordCramImage {
		var fonts:Array= new PFont[fontNames.length];
	...
	/**
	 * Make the WordCram render all words in the font that matches the given
	 * name, via Processing's <a
	 * href="http://processing.org/reference/createFont_.html"
	 * target="blank">createFont</a>.
	 * 
	 * @param fontName
	 *            the font name to pass to createFont.
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withFont(fontName:String):WordCramImage {
		var font:PFont= parent.createFont(fontName, 1);
		return withFont(font);
	}

	/**
	 * This WordCram will render words in one of the given <a
	 * href="http://processing.org/reference/PFont.html"
	 * target="blank">PFonts</a>.
	 * 
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withFonts( ... fonts):WordCramImage {
		return withFonter(Fonters.pickFrom(fonts));
	}

	/**
	 * Make the WordCram render all words in the given <a
	 * href="http://processing.org/reference/PFont.html"
	 * target="blank">PFont</a>.
	 * 
	 * @param font
	 *            the PFont to render the words in.
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withFont(font:PFont):WordCramImage {
	 ...
	 */
	public function withFonter(fonter:WordFonter):WordCramImage {
		this.fonter = fonter;
		return this;
	}
	...
	public function sizedByWeight(minSize:int, maxSize:int):WordCramImage {
	...
	public function sizedByRank(minSize:int, maxSize:int):WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withSizer(sizer:WordSizer):WordCramImage {
		this.sizer = sizer;
		return this;
	}

	/**
	 * Render words by randomly choosing from the given colors. Uses
	 * {@link Colorers#pickFrom(int...)}.
	 * 
	 * <p>
	 ...
	 * target="blank">Processing stores colors as integers</a>, WordCram will
	 * see each integer as a different color, and it'll color about 1/3 of your
	 * words with the color represented by the integer 255, and the other 2/3
	 * with the color represented by the integer 0. The punchline is, Processing
	 * stores opacity (or alpha) in the highest bits (the ones used for storing
	 * really big numbers, from 2<sup>24</sup> to 2<sup>32</sup>), so your
	 * colors 0 and 255 have, effectively, 0 opacity -- they're completely
	 * transparent. Oops.
	 * 
	 * <p>
	 ...
	 * 
	 * @param colors
	 *            the colors to randomly choose from.
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withColors( ... colors):WordCramImage {
	 ...
	 * @see #withColors(int...)
	 * @param color
	 *            the color for each word.
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withColor(color:int):WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withColorer(colorer:WordColorer):WordCramImage {
	 ...
	 *            The list of possible rotation angles, in radians
	 * @return The WordCram, for further setup or drawing.
	 */
	public function angledAt( ... anglesInRadians):WordCramImage {
	 ...
	 * 
	 * @param minAngleInRadians
	 *            The minimum rotation angle, in radians
	 * @param maxAngleInRadians
	 *            The maximum rotation angle, in radians
	 * @return The WordCram, for further setup or drawing.
	 */
	public function angledBetween(minAngleInRadians:Number,
			maxAngleInRadians:Number):WordCramImage {
		return withAngler(Anglers.randomBetween(minAngleInRadians,
				maxAngleInRadians));
	}
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withAngler(angler:WordAngler):WordCramImage {
		this.angler = angler;
		return this;
	}
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withPlacer(placer:WordPlacer):WordCramImage {
		this.placer = placer;
		return this;
	}
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withNudger(nudger:WordNudger):WordCramImage {
		this.nudger = nudger;
		return this;
	}
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function maxAttemptsToPlaceWord(maxAttempts:int):WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function maxNumberOfWordsToDraw(maxWords:int):WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function minShapeSize(minShapeSize:int):WordCramImage {
		renderOptions.minShapeSize = minShapeSize;
		return this;
	}

	/**
	 * Use a custom canvas instead of the applet's default one. This may be
	 * needed if rendering in background or in other dimensions than the applet
	 * size is needed.
	 * 
	 * @param canvas
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withCustomCanvas(canvas:PGraphics):WordCramImage {
	 ...
	 * @return The WordCram, for further setup or drawing.
	 */
	public function withWordPadding(padding:int):WordCramImage {
	...
	public function getWordCramEngine():WordCramEngine {
		if (wordCramEngine == null) {
			...
			if (words == null && textSource != null) {
				var text:String= textSource.getText();
			...
			if (sizer == null)
				sizer = Sizers.byWeight(5, 70);
			if (colorer == null)
				colorer = Colorers.alwaysUse(parent.color(0));
			if (angler == null)
				angler = Anglers.mostlyHoriz();
			if (placer == null)
				placer = Placers.horizLine();
			if (nudger == null)
				nudger = new SpiralWordNudger();

			var canvas:PGraphics= destination;
			wordCramEngine = new WordCramEngine(canvas, words, fonter, sizer,
					colorer, angler, placer, nudger, new WordShaper(),
					new BBTreeBuilder(), renderOptions);
		}
	...
	public Word[] getWords() {
		var wordsCopy:Array= new Word[words.length];
	 ...
	 *         one
	 */
	public function getWordAt(x:Number, y:Number):Word {
	...
	public function saveToFile(path:String):void {
		destination.save(parent.savePath(path));
	}

	public function getApplet():PApplet {
		return parent;
	}

	public function reset():void {
		destination.beginDraw();
		destination.background(0, 0); // grey level (black), alpha (transparent)
		// .. draw new content
		destination.endDraw();
		wordCramEngine = null;
		words = null;
