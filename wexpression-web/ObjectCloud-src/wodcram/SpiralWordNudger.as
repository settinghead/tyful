...
public class SpiralWordNudger implements WordNudger {
	...
	private var thetaIncrement:Number= float((Math.PI * 0.03));

	public function nudgeFor(w:Word, attempt:int):PVector {
		var rad:Number= powerMap(0.6, attempt, 0, 600, 1, 100);

		thetaIncrement = powerMap(1, attempt, 0, 600, 0.5, 0.3);
		var theta:Number= thetaIncrement * attempt;
		var x:Number= PApplet.cos(theta) * rad;
		var y:Number= PApplet.sin(theta) * rad;
	...
	private function powerMap(power:Number, v:Number, min1:Number, max1:Number,
		...
		var val:Number= PApplet.norm(v, min1, max1);
