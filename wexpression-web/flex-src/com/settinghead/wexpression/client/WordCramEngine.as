...
//		 frame.setVisible(true);
	...
	private function placeWord(eWord:EngineWord):Boolean {
		...
		var rect:Rectangle= eWord.getShape().getBounds2D(); // TODO can we move
															// these into
															// EngineWord.setDesiredLocation?
															// Does that make
															// sense?
		var wordImageWidth:int= int(rect.width);
		var wordImageHeight:int= int(rect.height);
...
//			eWord.getTree().draw(destination.graphics);
			...
			if (lastCollidedWith != null && eWord.overlaps(lastCollidedWith)) {
				var rectt:Rectangle= eWord.getShape().getBounds2D();
				continue;
			}
	...
	function getWordAt(x:Number, y:Number):Word {
		for (var i:int= 0; i < eWords.length; i++) {
			if (eWords[i].wasPlaced()) {
				var shape:ImageShape = eWords[i].getShape();
